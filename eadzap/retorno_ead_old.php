...
if($_GET['ver']=="sim"){
	...
	*/
	...
	if($method == 'GET' or $method == 'POST'){
		...
		/*
		$arquivo = "conteudo-json.txt";
		$fp = fopen($arquivo, "a+");
		fwrite($fp, $requestBody."
		");*/
		...
		if($lendo->token==$token_wordpress and $lendo->acao=="inscricao"){
			...
			$ins = $wpdb->query("delete from ead_zap_inscritos where id_curso=".$lendo->courseid." and iduser=".$lendo->userid.";");
			$ins = $wpdb->query("delete from ead_zap_controle_msg where id_curso=".$lendo->courseid." and id_user=".$lendo->userid.";");
			$ins = $wpdb->query("delete from ead_zap_realizar_curso where id_curso=".$lendo->courseid." and id_user=".$lendo->userid.";");
